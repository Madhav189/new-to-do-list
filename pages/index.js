...
export default function Home() {
  ...
  const toggleComplete = async (todo) => {
    ...
    if(newStatus) toast.success('Pulse completed! Great job.');
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-[conic-gradient(at_top_right,_var(--tw-gradient-stops))] from-slate-900 via-indigo-950 to-slate-900 px-4">
        ...
        <div className="bg-white/10 backdrop-blur-lg p-8 rounded-2xl shadow-2xl w-full max-w-md border border-white/10 relative overflow-hidden">
           ...
           <div className="text-center mb-8">
             <h1 className="text-5xl font-black text-white mb-2 tracking-tight">PulsePlan<span className="text-cyan-400">.</span></h1>
             <p className="text-indigo-200 text-sm">Organize your life rhythm.</p>
           </div>
           <div className="space-y-4">
             <input className="w-full px-4 py-3 bg-slate-900/60 border border-slate-600 rounded-xl text-white outline-none focus:ring-2 focus:ring-cyan-500 transition placeholder-slate-400" placeholder="Username" value={auth.username} onChange={e => setAuth({...auth, username: e.target.value})} />
             <input type="password" className="w-full px-4 py-3 bg-slate-900/60 border border-slate-600 rounded-xl text-white outline-none focus:ring-2 focus:ring-cyan-500 transition placeholder-slate-400" placeholder="Password" value={auth.password} onChange={e => setAuth({...auth, password: e.target.value})} />
             <button onClick={handleAuth} className="w-full bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white py-3 rounded-xl font-bold shadow-lg shadow-cyan-500/20 transition transform hover:scale-[1.02]">{isSignUpMode ? 'Get Started' : 'Sign In'}</button>
             <button onClick={() => setIsSignUpMode(!isSignUpMode)} className="w-full text-slate-400 text-sm hover:text-white transition">{isSignUpMode ? 'Already have an account? Sign In' : "New to PulsePlan? Create account"}</button>
           </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className={`${theme.cardBg} p-6 rounded-2xl shadow-lg mb-8 border ${theme.cardBorder} relative`}>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {/* PRIORITY DROPDOWN */}
              <select 
                className={`px-4 py-3 ${theme.inputBg} border ${theme.inputBorder} rounded-xl focus:ring-2 focus:ring-cyan-500 outline-none ${theme.subText}`}
                value={form.priority}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className={t.is_completed ? 'opacity-40 grayscale transition duration-500' : ''}>
                  <div className={`font-semibold text-lg ${t.is_completed ? 'line-through decoration-2 decoration-slate-600' : theme.text} flex items-center gap-2`}>
                    {t.task}
                    {/* Priority Badge */}
                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${getPriorityColor(t.priority || 'medium')}`}>
                      {t.priority || 'medium'}
                    </span>
                  </div>
